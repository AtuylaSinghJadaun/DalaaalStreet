      ...
      {teams.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {sortedTeams.map((team) => {
            // A team has logged in if any participant ever signed in — marked by
            // ipo_participant, or proven by holding shares.
            const loggedIn = team.ipo_participant || holdings.some(h => h.team_id === team.id && h.quantity > 0)
            return (
            <div
              key={team.id}
            ...
            >
              ...
              <div className="flex min-h-[2.75rem] items-start justify-between gap-3">
                ...
                <span className={loggedIn ? 'pill pill-pos' : 'pill pill-muted'}>
              ...
              <div className="my-4 space-y-2.5 border-y border-white/[0.06] py-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-display text-base font-semibold tabular-nums text-[#76e9a8]">
                    ₹{team.cash_balance.toLocaleString('en-IN')}
                  </span>
                </div>
              </div>
